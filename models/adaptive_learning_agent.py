...
class AdaptiveLearningAgent:
    ...
    def __init__(self, lookback_window: int = 100):
        ...
        # Adaptive thresholds (learned online)
        self.confidence_threshold = 0.65  # Initial value
    ...
    def adapt_confidence_threshold(self):
        ...
        # Adapt threshold
        if recent_win_rate < 0.4:  # Losing streak
            self.confidence_threshold = min(
                self.max_confidence,
                self.confidence_threshold + 0.02
            )
        elif recent_win_rate > 0.6:  # Winning streak
    ...
    def calibrate_confidence(self, base_confidence: float, regime: str, signal: str, 
        ...
        regime_adjustment = (regime_win_rate - 0.5) * 0.2  # ±0.1 max
        calibrated += regime_adjustment
        
        # Adjust based on signal-specific win rate
        signal_win_rate = self.get_signal_win_rate(signal)
        signal_adjustment = (signal_win_rate - 0.5) * 0.15  # ±0.075 max
        calibrated += signal_adjustment
        
        # Adjust based on momentum/pattern alignment
        if signal == 'LONG' and market_patterns['momentum_strength'] > 0.6:
            calibrated += 0.05  # Momentum aligned with signal
        elif signal == 'SHORT' and market_patterns['momentum_strength'] > 0.6:
            calibrated += 0.05
        
        # Penalize if reversal likely
        if market_patterns['reversal_probability'] > 0.7:
            calibrated -= 0.1
        
        # Boost if continuation likely
        if market_patterns['continuation_probability'] > 0.7:
            calibrated += 0.05
