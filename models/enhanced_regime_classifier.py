...
class EnhancedRegimeClassifier:
    ...
    def __init__(self, features: pd.DataFrame):
        ...
        # Fuzzy membership thresholds (ADJUSTED FOR LOW-VOLATILITY MARKETS)
        # Previous values were too high for current BTC market conditions
        self.volatility_low = 0.002   # Was 0.008 - now detects trends in calmer markets
        self.volatility_medium = 0.005  # Was 0.015 - more sensitive
        self.volatility_high = 0.010   # Was 0.025 - lower threshold for trend detection
        
        # Regime persistence (prevent rapid switching) - REDUCED FOR FASTER RESPONSE
        self.last_regime = None
        self.regime_persistence_count = 0
        self.min_persistence = 2  # Minimum ticks before regime change (was 3, now more responsive)
    ...
    def compute_regime_scores(self, row: pd.Series) -> Dict[str, float]:
        ...
        # TREND_UP: PRIORITIZE POSITIVE MOMENTUM (even with lower volatility)
        scores['TREND_UP'] = (
            vol_membership['medium'] * 0.3 +  # Accept medium volatility (was high only)
            vol_membership['high'] * 0.2 +    # Still reward high volatility
            max(0, returns * 200) * 0.4 +     # INCREASED: Positive returns boost (was 0.3, scale x2)
            max(0, volume_accel) * 0.05 +     # Reduced volume weight (was 0.2)
            (1 / (1 + np.exp(-stability))) * 0.05  # Reduced stability weight
        )
        
        # TREND_DOWN: PRIORITIZE NEGATIVE MOMENTUM (even with lower volatility)
        scores['TREND_DOWN'] = (
            vol_membership['medium'] * 0.3 +  # Accept medium volatility (was high only)
            vol_membership['high'] * 0.2 +    # Still reward high volatility
            max(0, -returns * 200) * 0.4 +    # INCREASED: Negative returns boost (was 0.3, scale x2)
            max(0, volume_accel) * 0.05 +     # Reduced volume weight
            (1 / (1 + np.exp(-stability))) * 0.05
        )
